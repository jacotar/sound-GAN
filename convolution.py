...
class Convolution:
	...
	def init_random(self):
		self.filters = [theano.shared((
			...
			self.maxs[i] * self.dims[i+1] * self.mults[i])-0.5)/(self.dims[i] * self.wins[i] * 0.1))
...
class Deconvolution:
	...
	def apply(self, x):
		...
		for i in range(num):
			...
			y = T.reshape(T.dot(y, self.filters[i]), 
				[batch_size, k * wins[i], 
			...
			for j in range(mults[i]):
				y = T.inc_subtensor(y[:, j*wins[i]:(k+j)*wins[i], :], old_y[:, :, maxs[i] * dims[i+1] * j : maxs[i] * dims[i+1] * (j+1)])
'''
